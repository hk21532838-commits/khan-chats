...
const msgMenuRef=useRef(null);const aiEndRef=useRef(null);const reactBarRef=useRef(null);
...
useEffect(()=>{
  const h=e=>{
    ...
    if(reactBar&&reactBarRef.current&&!reactBarRef.current.contains(e.target))setReactBar(null);
  };
...
useEffect(()=>{
  const unsub=onAuthStateChanged(auth,async u=>{
    if(u){setUser(u);setScreen("chat");setNewName(u.displayName||"");
...
const loadContactProfile=async(contact)=>{
  ...
  try{
    ...
    const u=await get(ref(db,"usernames/"+contact.uid));if(u.val())setContactUname(u.val());
...
const addReaction=async(msgKey,emoji)=>{
  ...
  const msgReacts=reactions[msgKey]||{};
  const cur=msgReacts[user.uid];
...
const saveProfile=async()=>{
  ...
  setUser({...user,displayName:newName.trim()});alert("Profile updated!");
};
...
const openChat=c=>{
  setActiveChat(c);setNav("chat");setShowEmoji(false);setReplyTo(null);setMsgMenu(null);setReactBar(null);setShowStarred(false);setShowDisappear(false);
...
const getMsgReactions=msgKey=>{const r=reactions[msgKey]||{};const counts={};Object.values(r).forEach(e=>{counts[e]=(counts[e]||0)+1;});return counts;};
const getMyReaction=msgKey=>{return(reactions[msgKey]||{})[user?.uid]||null;};

const Inp=({value,onChange,placeholder,type="text",style={},onKeyDown,autoFocus})=>(<input value={value} onChange={onChange} placeholder={placeholder} type={type} onKeyDown={onKeyDown} autoFocus={autoFocus} style={{padding:"12px 16px",background:T.card,border:"1.5px solid "+T.border,borderRadius:14,color:T.text,fontSize:14,outline:"none",fontFamily:"'Inter',sans-serif",transition:"all 0.2s",width:"100%",boxSizing:"border-box",...style}} onFocus={e=>{e.target.style.borderColor=T.blue;e.target.style.boxShadow="0 0 0 3px rgba(79,142,247,0.12)";}} onBlur={e=>{e.target.style.borderColor=T.border;e.target.style.boxShadow="none";}} />);
const Btn=({children,onClick,v="p",style={}})=>(<div onClick={onClick} style={{padding:"12px 18px",background:v==="d"?T.gradD:v==="g"?T.card2:v==="s"?T.gradG:T.grad,borderRadius:14,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",textAlign:"center",userSelect:"none",fontFamily:"'Poppins',sans-serif",...style}} onMouseDown={e=>e.currentTarget.style.transform="scale(0.96)"} onMouseUp={e=>e.currentTarget.style.transform="scale(1)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>{children}</div>);
...
{policy&&<Modal onClose={()=>setPolicy(null)}><Card style={{padding:26,maxHeight:"75vh",overflowY:"auto"}}><div style={{fontWeight:800,fontSize:17,color:T.text,marginBottom:12}}>{policy==="privacy"?"🔒 Privacy":"📋 Terms"}</div><div style={{color:T.mutedL,fontSize:13,lineHeight:1.9}}>{policy==="privacy"?<><p>Independent platform.</p><p><strong style={{color:T.text}}>Data:</strong> Email, name, photo, messages.</p></>:<><p>Lawful use only.</p><p>Not affiliated with WhatsApp or Meta.</p></>}</div><Btn onClick={()=>setPolicy(null)} style={{marginTop:16}}>Close</Btn></Card></Modal>}
{showStarred&&<Modal onClose={()=>setShowStarred(false)}><Card style={{padding:0,maxHeight:"80vh",overflow:"hidden",display:"flex",flexDirection:"column"}}><div style={{padding:"18px 20px",borderBottom:"1px solid "+T.border,display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{fontWeight:800,fontSize:17,color:T.text}}>⭐ Starred Messages</div><div onClick={()=>setShowStarred(false)} style={{color:T.muted,cursor:"pointer",fontSize:20}}>✕</div></div><div style={{flex:1,overflowY:"auto",padding:14}}>{starredList.length===0?<div style={{textAlign:"center",padding:40,color:T.muted}}><div style={{fontSize:44,marginBottom:10}}>⭐</div><div style={{fontWeight:700,fontSize:14,color:T.text}}>No starred messages</div><div style={{fontSize:12,marginTop:6}}>Tap message menu → ⭐ Star</div></div>:starredList.map((s,i)=>(<div key={i} style={{padding:"12px 14px",background:T.card2,borderRadius:14,marginBottom:8,border:"1px solid "+T.border}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><div style={{width:24,height:24,borderRadius:8,background:cfn(s.chatName),display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,color:"#fff",flexShrink:0}}>{gi(s.chatName)}</div><div style={{fontSize:11,color:T.blue,fontWeight:700}}>{s.chatName}</div><div style={{fontSize:10,color:T.muted,marginLeft:"auto"}}>{tAgo(s.timestamp)}</div></div><div style={{fontSize:13,color:T.text,fontWeight:500}}>{s.text||"📷 Photo"}</div><div style={{fontSize:10,color:T.muted,marginTop:4}}>From: {s.senderName}</div></div>))}</div></Card></Modal>}
{showDisappear&&activeChat&&<Modal onClose={()=>setShowDisappear(false)}><Card style={{padding:24}}><div style={{fontWeight:800,fontSize:17,color:T.text,marginBottom:6}}>⏱️ Disappearing Messages</div><div style={{fontSize:12,color:T.muted,marginBottom:18}}>Messages auto-delete after selected time</div>{DISAPPEAR_OPTS.map(opt=>(<div key={opt.val} onClick={()=>setDisappearTime(activeChat.chatId,opt.val)} style={{padding:"13px 16px",background:curDisappear===opt.val?T.grad:T.card2,borderRadius:13,marginBottom:8,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",border:"1.5px solid "+(curDisappear===opt.val?T.blue:T.border),transition:"all 0.2s"}}><span style={{fontWeight:700,fontSize:14,color:"#fff"}}>{opt.label}</span>{curDisappear===opt.val&&<span style={{color:"#fff",fontSize:16}}>✓</span>}</div>))}<Btn onClick={()=>setShowDisappear(false)} v="g" style={{marginTop:6}}>Cancel</Btn></Card></Modal>}
...
<div style={{fontSize:10,fontWeight:600,marginTop:1,color:isTyping?"#A78BFA":T.blue,display:"flex",alignItems:"center",gap:4}}>
{curDisappear>0&&<span style={{background:"rgba(79,142,247,0.15)",borderRadius:6,padding:"1px 5px",fontSize:9}}>⏱️ {DISAPPEAR_OPTS.find(o=>o.val===curDisappear)?.label}</span>}
...
<div style={{flex:1,overflowY:"auto",padding:"14px 12px",display:"flex",flexDirection:"column",gap:2,position:"relative",...bgStyle()}} onClick={()=>{setMsgMenu(null);setShowEmoji(false);setReactBar(null);setHoveredMsg(null);}}>
...
const isReactBarOpen=reactBar&&reactBar.msgKey===msgKey;
return(
<div key={i}>
...
<div style={{display:"flex",justifyContent:isMine?"flex-end":"flex-start",marginBottom:hasReacts?14:(showAv?5:2),animation:"msgIn 0.2s ease",position:"relative",alignItems:"flex-end"}}>
...
{isMine&&!isDeleted&&(
<div onClick={e=>{e.stopPropagation();setReactBar(reactBar&&reactBar.msgKey===msgKey?null:{msgKey,isMine,msg});}} style={{width:28,height:28,borderRadius:"50%",background:isReactBarOpen?T.grad:T.card2,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:14,border:"1px solid "+T.border,marginRight:6,marginBottom:4,transition:"all 0.2s",flexShrink:0,opacity:1}}>😊</div>
)}

<div style={{maxWidth:"72%",display:"flex",flexDirection:"column",alignItems:isMine?"flex-end":"flex-start",position:"relative"}}>
...
{!isMine&&!isDeleted&&(
<div onClick={e=>{e.stopPropagation();setReactBar(reactBar&&reactBar.msgKey===msgKey?null:{msgKey,isMine,msg});}} style={{width:28,height:28,borderRadius:"50%",background:isReactBarOpen?T.grad:T.card2,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:14,border:"1px solid "+T.border,marginLeft:6,marginBottom:4,transition:"all 0.2s",flexShrink:0,opacity:1}}>😊</div>
)}
</div>

{isReactBarOpen&&(
<div style={{display:"flex",justifyContent:isMine?"flex-end":"flex-start",marginBottom:4,paddingRight:isMine?0:36,paddingLeft:isMine?36:36}}>
<div ref={reactBarRef} style={{background:T.card,borderRadius:22,padding:"8px 10px",border:"1px solid "+T.border,boxShadow:"0 4px 24px rgba(0,0,0,0.6)",display:"flex",gap:4,alignItems:"center",animation:"reactBarIn 0.2s ease",flexWrap:"wrap",maxWidth:280}}>
{REACT_EMOJIS.map(emoji=>(
<div key={emoji} onClick={e=>{e.stopPropagation();addReaction(msgKey,emoji);}} style={{width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,cursor:"pointer",borderRadius:12,background:myReact===emoji?T.grad:"transparent",transition:"all 0.15s",border:myReact===emoji?"1px solid "+T.blue:"1px solid transparent"}} onTouchStart={e=>{e.currentTarget.style.transform="scale(1.3)";}} onTouchEnd={e=>{e.currentTarget.style.transform="scale(1)";}}>{emoji}</div>
))}
<div style={{width:1,height:24,background:T.border,margin:"0 2px"}} />
{[["↩️",()=>{setReplyTo({text:reactBar.msg?.text||"Photo",senderName:reactBar.msg?.senderName||""});setReactBar(null);}],["⭐",()=>{toggleStar(msgKey,msg);setReactBar(null);}],...(msg.text?[["📋",()=>{copyMsg(msg.text);setReactBar(null);}]]:[]),...(isMine?[["🗑️",()=>{deleteMsg(msgKey,false);setReactBar(null);}]]:[] )].map(([icon,fn])=>(
<div key={icon} onClick={e=>{e.stopPropagation();fn();}} style={{width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,cursor:"pointer",borderRadius:10,background:T.card2,transition:"all 0.15s"}}>{icon}</div>
))}
</div>
</div>
)}
...
{msgMenu&&(
<div ref={msgMenuRef} style={{position:"fixed",bottom:90,left:"50%",transform:"translateX(-50%)",background:T.card,borderRadius:18,padding:8,border:"1px solid "+T.border,boxShadow:"0 8px 32px rgba(0,0,0,0.7)",zIndex:500,minWidth:200,animation:"slideUp 0.2s ease"}}>
{[["↩️ Reply",()=>{setReplyTo({text:msgMenu.msg?.text||"Photo",senderName:msgMenu.msg?.senderName||""});setMsgMenu(null);}],["⭐ Star/Unstar",()=>{toggleStar(msgMenu.msgKey,msgMenu.msg);setMsgMenu(null);}],...(msgMenu.msg?.text?[["📋 Copy",()=>copyMsg(msgMenu.msg.text)]]:[]),...(msgMenu.isMine?[["🗑️ Delete for me",()=>deleteMsg(msgMenu.msgKey,false)],["🚫 Delete for all",()=>deleteMsg(msgMenu.msgKey,true)]]:[["🗑️ Delete for me",()=>deleteMsg(msgMenu.msgKey,false)]]),["✕ Close",()=>setMsgMenu(null)]].map(([label,fn])=>(
<div key={label} onClick={fn} style={{padding:"12px 16px",borderRadius:12,cursor:"pointer",fontSize:13,fontWeight:600,color:(label.includes("Delete")||label.includes("🚫"))?"#EF4444":T.text,transition:"background 0.15s"}} onMouseEnter={e=>e.currentTarget.style.background=T.card2} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>{label}</div>
))}
</div>
)}
