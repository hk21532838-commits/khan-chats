...
useEffect(()=>{
  const handleClick=e=>{if(msgMenu&&msgMenuRef.current&&!msgMenuRef.current.contains(e.target))setMsgMenu(null);};
...
const sendMsg=async(imgData=null)=>{
  ...
  const msg={
    ...
    timestamp:Date.now(),
    status:"sent",
    ...(replyTo?{replyTo:{text:replyTo.text||"📷 Photo",senderName:replyTo.senderName,msgId:replyTo.msgId}}:{})
  };
...
const deleteMsg=async(msgId,forEveryone=false)=>{
  if(forEveryone){
...
const copyMsg=text=>{navigator.clipboard.writeText(text).then(()=>alert("Copied!"));setMsgMenu(null);};
...
const toggleRecording=()=>{
  if(isRecording){
    setIsRecording(false);clearInterval(recTimer.current);setRecTime(0);
...
const Av=({name,p,size=44})=>(
  p?<img src={p} alt="a" style={{width:size,height:size,borderRadius:size*0.33,objectFit:"cover",flexShrink:0}} />
  :<div style={{width:size,height:size,borderRadius:size*0.33,background:T.grad,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:size*0.3,color:"#fff",flexShrink:0}}>{gi(name)}</div>
);
...
const Btn=({children,onClick,v="primary",style={}})=>(
  <div onClick={onClick} style={{padding:"12px 18px",background:v==="danger"?T.gradD:v==="ghost"?T.card2:v==="success"?T.gradG:T.grad,borderRadius:14,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",textAlign:"center",boxShadow:v==="ghost"?"none":T.shadow,transition:"transform 0.15s",userSelect:"none",fontFamily:"'Poppins',sans-serif",...style}}
...
const Toggle=({val,onToggle})=>(
  <div onClick={onToggle} style={{width:50,height:27,borderRadius:14,background:val?T.grad:T.card2,position:"relative",cursor:"pointer",transition:"all 0.3s",border:`1px solid ${T.border}`,flexShrink:0}}>
    <div style={{position:"absolute",top:3,left:val?24:3,width:19,height:19,borderRadius:"50%",background:"#fff",transition:"all 0.3s",boxShadow:"0 2px 6px rgba(0,0,0,0.3)"}} />
  </div>
);
...
const chatBgStyle=()=>{
  ...
  if(chatBg==="waves")return{backgroundImage:`url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z' fill='rgba(79,142,247,0.05)' fill-rule='nonzero'/%3E%3C/g%3E%3C/svg%3E")`};
  return{};
};

// DATE SEPARATOR
const getDaySep=(msgs,idx)=>{
  if(idx===0)return true;
  const cur=new Date(msgs[idx].timestamp);const prev=new Date(msgs[idx-1].timestamp);
...
if(screen==="login"||screen==="register")return(
  <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:T.bg,fontFamily:"'Poppins',sans-serif",padding:20,animation:"fadeIn 0.5s ease"}}>
    ...
    <div style={{width:"100%",maxWidth:420}}>
      ...
      <Card style={{padding:5,marginBottom:24}}>
        <div style={{display:"flex",borderRadius:17}}>
          {["login","register"].map(s=>(
            <div key={s} onClick={()=>{setScreen(s);setAErr("");}} style={{flex:1,textAlign:"center",padding:"13px",cursor:"pointer",fontWeight:700,fontSize:14,background:screen===s?T.grad:"transparent",color:screen===s?"#fff":T.muted,borderRadius:15,margin:2,transition:"all 0.3s"}}>
      ...
      <Btn onClick={screen==="login"?login:register} style={{marginTop:16,fontSize:15,padding:"15px",borderRadius:18,boxShadow:T.shadowL}}>
    ...
    {policy&&<Modal onClose={()=>setPolicy(null)}>
      <Card style={{padding:28,maxHeight:"78vh",overflowY:"auto"}}>
        ...
        <div style={{color:T.mutedL,fontSize:13,lineHeight:1.9}}>
          ...
          :<><p>By using Khan Chats you agree.</p><p><strong style={{color:T.text}}>Use:</strong> Lawful communication only.</p><p><strong style={{color:T.text}}>Disclaimer:</strong> Not affiliated with WhatsApp or Meta.</p></>}
        </div>
...
if(showSett)return(
  <div style={{position:"fixed",inset:0,background:T.bg,zIndex:9999,display:"flex",flexDirection:"column",fontFamily:"'Inter',sans-serif",color:T.text,animation:"slideR 0.3s ease"}}>
    ...
    <div style={{display:"flex",background:T.card,borderBottom:`1px solid ${T.border}`,overflowX:"auto",padding:"0 6px"}}>
      {[["profile","👤","Profile"],["privacy","🔒","Privacy"],["notifs","🔔","Notifs"],["lang","🌐","Language"],["chat","🎨","Chat"],["ai","🤖","AI"],["legal","📋","Legal"]].map(([tab,icon,label])=>(
        <div key={tab} onClick={()=>setSTab(tab)} style={{padding:"11px 13px",cursor:"pointer",fontSize:11,fontWeight:700,whiteSpace:"nowrap",color:sTab===tab?T.blue:T.muted,borderBottom:sTab===tab?`2.5px solid ${T.blue}`:"2.5px solid transparent",transition:"all 0.2s"}}>
          {icon} {label}
        </div>
      ))}
    </div>
    <div style={{flex:1,overflowY:"auto",padding:18,display:"flex",flexDirection:"column",gap:14}}>
      ...
      {sTab==="profile"&&<div style={{animation:"slideUp 0.3s ease"}}>
        ...
        <Card style={{padding:18,marginBottom:14}}>
          <div style={{fontSize:10,color:T.blue,fontWeight:700,marginBottom:12,textTransform:"uppercase",letterSpacing:1.5}}>Chat Locks (PIN)</div>
          {Object.entries(contacts).length===0?<div style={{color:T.muted,fontSize:13,textAlign:"center",padding:14}}>No contacts yet</div>
          :Object.entries(contacts).map(([chatId,c])=>(
            <div key={chatId} style={{display:"flex",alignItems:"center",padding:"10px 12px",background:T.card2,borderRadius:13,marginBottom:8,gap:11,border:`1px solid ${T.border}`}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:locks[chatId]?"#EF4444":T.muted,fontWeight:600}}>{locks[chatId]?"🔒 Locked":"🔓 Unlocked"}</div>
              </div>
              {locks[chatId]
                ?<div onClick={()=>{if(window.confirm("Remove lock?"))removeLock(chatId);}} style={{padding:"6px 12px",background:"rgba(239,68,68,0.1)",borderRadius:10,color:"#EF4444",fontSize:12,fontWeight:700,cursor:"pointer"}}>Remove</div>
                :<div onClick={()=>{setLModal(chatId);setLPin("");setLErr("");}} style={{padding:"6px 14px",background:T.grad,borderRadius:10,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>🔒 Lock</div>}
            </div>
          ))}
        </Card>
      ...
      {sTab==="privacy"&&<div style={{animation:"slideUp 0.3s ease"}}>
        {[["Last Seen","Show last active time",true],["Online Status","Show when online",true],["Read Receipts","Show read ticks",true],["Profile Photo","Who can see your photo",true]].map(([t,d,v],i)=>(
          <Card key={i} style={{padding:"15px 18px",marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{flex:1}}><div style={{fontWeight:700,fontSize:13,color:T.text}}>{t}</div><div style={{fontSize:12,color:T.muted,marginTop:2}}>{d}</div></div><Toggle val={v} onToggle={()=>{}} /></div>
          </Card>
        ))}
        <Card style={{padding:"15px 18px"}}><div style={{fontWeight:700,fontSize:13,color:T.text,marginBottom:8}}>Blocked Contacts</div><div style={{color:T.muted,fontSize:13,textAlign:"center",padding:10}}>No blocked contacts</div></Card>
      </div>}

      {sTab==="notifs"&&<div style={{animation:"slideUp 0.3s ease"}}>
        {[["Messages","Message notifications","msgs"],["Updates","Status notifications","updates"],["Calls","Call notifications","calls"]].map(([t,d,k])=>(
          <Card key={k} style={{padding:"15px 18px",marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{flex:1}}><div style={{fontWeight:700,fontSize:13,color:T.text}}>{t}</div><div style={{fontSize:12,color:T.muted,marginTop:2}}>{d}</div></div><Toggle val={nSett[k]} onToggle={()=>setNSett(p=>({...p,[k]:!p[k]}))} /></div>
          </Card>
        ))}
        <Card style={{padding:"15px 18px",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{flex:1}}><div style={{fontWeight:700,fontSize:13,color:T.text}}>Dark Mode</div><div style={{fontSize:12,color:T.muted,marginTop:2}}>Toggle theme</div></div><Toggle val={darkMode} onToggle={()=>setDarkMode(p=>!p)} /></div>
        </Card>
      </div>}
      ...
      {sTab==="chat"&&<div style={{animation:"slideUp 0.3s ease"}}>
        <Card style={{padding:18,marginBottom:14}}>
          ...
          {[["none","No Pattern"],["dots","Dots"],["grid","Grid"],["waves","Pattern"]].map(([val,label])=>(
      ...
      {sTab==="ai"&&<div style={{display:"flex",flexDirection:"column",minHeight:"55vh",animation:"slideUp 0.3s ease"}}>
        <div style={{background:T.grad,borderRadius:22,padding:22,marginBottom:16,textAlign:"center",boxShadow:T.shadowL}}>
          ...
          <div style={{fontSize:12,color:"rgba(255,255,255,0.65)",marginTop:4}}>Powered by Claude · Ask anything</div>
        </div>
        <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:10,marginBottom:12,minHeight:180}}>
          {aiMsgs.length===0&&<div style={{textAlign:"center",color:T.muted,padding:36}}><div style={{fontSize:44,marginBottom:10}}>✨</div><div style={{fontWeight:700,fontSize:15,color:T.text}}>Ask anything!</div></div>}
          {aiMsgs.map((m,i)=>(
            <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",animation:"msgIn 0.25s ease"}}>
              <div style={{maxWidth:"85%",padding:"12px 16px",background:m.role==="user"?T.grad:T.card,borderRadius:m.role==="user"?"18px 18px 5px 18px":"18px 18px 18px 5px",fontSize:14,color:T.text,lineHeight:1.7,boxShadow:m.role==="user"?T.shadow:T.cardShadow,border:m.role==="user"?"none":`1px solid ${T.border}`}}>{m.text}</div>
            </div>
          ))}
          {aiLoad&&<div style={{display:"flex",gap:6,padding:"12px 16px",background:T.card,borderRadius:"18px 18px 18px 5px",width:"fit-content",border:`1px solid ${T.border}`}}>{[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:T.blue,animation:`dot 1.4s ${i*0.2}s infinite`}} />)}</div>}
        </div>
        <div style={{display:"flex",gap:8}}>
          <Inp value={aiIn} onChange={e=>setAiIn(e.target.value)} onKeyDown={e=>e.key==="Enter"&&askAI()} placeholder="Ask Khan AI..." style={{flex:1}} />
          <div onClick={askAI} style={{width:48,height:48,borderRadius:15,background:aiIn.trim()?T.grad:T.card2,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:20,flexShrink:0,transition:"all 0.2s",boxShadow:aiIn.trim()?T.shadow:"none"}}>➤</div>
        </div>
      </div>}

      {sTab==="legal"&&<div style={{animation:"slideUp 0.3s ease"}}>
        {[["🔒 Privacy Policy","privacy"],["📋 Terms of Service","terms"],["📧 Contact Us","contact"],["❓ Help & FAQ","help"],["🗑️ Delete Account","delete"]].map(([title,action])=>(
          <Card key={action} style={{padding:"15px 18px",marginBottom:10,cursor:"pointer"}}
            onClick={()=>{if(action==="delete")setDeleteC(true);else if(action==="contact")alert("📧 khanchats.support@gmail.com");else if(action==="help")alert("❓ FAQ\n\n• Chat lock: Settings → Profile → Chat Locks\n• Invite friends: Tap 🔗 in header\n• Change language: Settings → Language");else setPolicy(action);}}>
        ...
        <Card style={{padding:18,marginTop:6,textAlign:"center"}}>
          ...
          <div style={{fontSize:11,color:T.muted,lineHeight:1.8}}>Independent Platform · Not affiliated with WhatsApp or Meta</div>
        </Card>
      </div>}
    </div>

    {logoutC&&<Modal onClose={()=>setLogoutC(false)}><Card style={{padding:30,textAlign:"center"}}><div style={{fontSize:46,marginBottom:12}}>🚪</div><div style={{fontWeight:800,fontSize:18,color:T.text,marginBottom:8,fontFamily:"'Poppins',sans-serif"}}>Sign Out?</div><div style={{color:T.muted,fontSize:13,marginBottom:20,lineHeight:1.6}}>Are you sure you want to sign out?</div><div style={{display:"flex",gap:10}}><Btn onClick={()=>setLogoutC(false)} v="ghost" style={{flex:1}}>Cancel</Btn><Btn onClick={logout} v="danger" style={{flex:1}}>Sign Out</Btn></div></Card></Modal>}
    {deleteC&&<Modal onClose={()=>setDeleteC(false)}><Card style={{padding:30,textAlign:"center"}}><div style={{fontSize:46,marginBottom:12}}>⚠️</div><div style={{fontWeight:800,fontSize:18,color:"#EF4444",marginBottom:8,fontFamily:"'Poppins',sans-serif"}}>Delete Account?</div><div style={{color:T.muted,fontSize:13,marginBottom:20,lineHeight:1.6}}>Permanent action. Cannot be undone.</div><div style={{display:"flex",gap:10}}><Btn onClick={()=>setDeleteC(false)} v="ghost" style={{flex:1}}>Cancel</Btn><Btn onClick={async()=>{await logout();setDeleteC(false);}} v="danger" style={{flex:1}}>Delete</Btn></div></Card></Modal>}
    {policy&&<Modal onClose={()=>setPolicy(null)}><Card style={{padding:26,maxHeight:"75vh",overflowY:"auto"}}><div style={{fontWeight:800,fontSize:17,color:T.text,marginBottom:12}}>{policy==="privacy"?"🔒 Privacy Policy":"📋 Terms"}</div><div style={{color:T.mutedL,fontSize:13,lineHeight:1.9}}>{policy==="privacy"?<><p>Khan Chats is independent and committed to privacy.</p><p><strong style={{color:T.text}}>Data:</strong> Email, name, photo, messages.</p><p><strong style={{color:T.text}}>Security:</strong> Firebase.</p><p><strong style={{color:T.text}}>Rights:</strong> Delete anytime.</p></>:<><p>By using Khan Chats you agree.</p><p><strong style={{color:T.text}}>Use:</strong> Lawful only.</p><p><strong style={{color:T.text}}>Disclaimer:</strong> Not affiliated with WhatsApp or Meta.</p></>}</div><Btn onClick={()=>setPolicy(null)} style={{marginTop:16}}>Close</Btn></Card></Modal>}
  </div>
);
...
if(lModal)return(
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:10001,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Poppins',sans-serif"}}>
    <style>{CSS}</style>
    <Card style={{padding:34,maxWidth:360,width:"100%",textAlign:"center",animation:"slideUp 0.3s ease"}}>
      <div style={{width:72,height:72,borderRadius:22,background:T.grad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 16px",boxShadow:T.shadow}}>🔒</div>
      <h3 style={{color:T.text,fontWeight:800,fontSize:20,marginBottom:6}}>Set Chat PIN</h3>
      <p style={{color:T.muted,fontSize:13,marginBottom:20}}>Min 4 digits required</p>
      <Inp value={lPin} onChange={e=>setLPin(e.target.value.replace(/\D/g,""))} placeholder="• • • •" type="password" style={{fontSize:24,textAlign:"center",letterSpacing:12,marginBottom:8}} />
      {lErr&&<div style={{color:"#EF4444",fontSize:13,marginBottom:10,padding:"7px",background:"rgba(239,68,68,0.08)",borderRadius:9}}>{lErr}</div>}
      <div style={{display:"flex",gap:10,marginTop:10}}><Btn onClick={()=>{setLModal(null);setLPin("");setLErr("");}} v="ghost" style={{flex:1}}>Cancel</Btn><Btn onClick={()=>lockChat(lModal)} style={{flex:1}}>Lock 🔒</Btn></div>
    </Card>
  </div>
);
...
if(ulModal)return(
  <div style={{position:"fixed",inset:0,background:T.bg,zIndex:10001,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Poppins',sans-serif"}}>
    ...
    <Card style={{padding:34,maxWidth:360,width:"100%",textAlign:"center",animation:"slideUp 0.3s ease"}}>
      <div style={{width:72,height:72,borderRadius:22,background:T.grad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 16px",boxShadow:T.shadow}}>🔐</div>
      <h3 style={{color:T.text,fontWeight:800,fontSize:20,marginBottom:6}}>Chat Locked</h3>
      <p style={{color:T.muted,fontSize:13,marginBottom:20}}>Enter PIN to unlock</p>
      <Inp value={ulPin} onChange={e=>setUlPin(e.target.value.replace(/\D/g,""))} placeholder="• • • •" type="password" style={{fontSize:24,textAlign:"center",letterSpacing:12,marginBottom:8}} onKeyDown={e=>e.key==="Enter"&&unlockChat(ulModal)} />
      {lErr&&<div style={{color:"#EF4444",fontSize:13,marginBottom:10,padding:"7px",background:"rgba(239,68,68,0.08)",borderRadius:9}}>{lErr}</div>}
      <div style={{display:"flex",gap:10,marginTop:10}}><Btn onClick={()=>{setUlModal(null);setUlPin("");setLErr("");}} v="ghost" style={{flex:1}}>Cancel</Btn><Btn onClick={()=>unlockChat(ulModal)} style={{flex:1}}>Unlock →</Btn></div>
    </Card>
  </div>
);
...
if(inCall)return(
  <div style={{position:"fixed",inset:0,background:T.bg,zIndex:9997,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:26,fontFamily:"'Poppins',sans-serif"}}>
    <style>{CSS}</style>
    <div style={{width:96,height:96,borderRadius:30,background:T.grad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:44,boxShadow:"0 0 60px rgba(79,142,247,0.5)",animation:"pulse 2s infinite"}}>{callType==="video"?"📹":"📞"}</div>
    <div style={{textAlign:"center"}}><div style={{color:T.text,fontSize:24,fontWeight:800,letterSpacing:"-0.5px"}}>{activeChat?.name}</div><div style={{color:T.muted,fontSize:13,marginTop:6}}>{callType==="video"?"Video":"Audio"} call...</div></div>
    {callType==="video"&&<div style={{display:"flex",gap:14}}><video ref={lvRef} autoPlay muted style={{width:150,height:114,borderRadius:17,background:T.card,border:`2px solid ${T.blue}`,objectFit:"cover"}} /><video ref={rvRef} autoPlay style={{width:150,height:114,borderRadius:17,background:T.card,border:`2px solid ${T.purple}`,objectFit:"cover"}} /></div>}
    <Btn onClick={endCall} v="danger" style={{padding:"15px 46px",borderRadius:22,fontSize:15,boxShadow:"0 8px 30px rgba(239,68,68,0.4)"}}>End Call</Btn>
  </div>
);
...
return(
  <div style={{display:"flex",flexDirection:"column",height:"100vh",fontFamily:"'Inter',sans-serif",background:T.bg,color:T.text,overflow:"hidden",animation:"fadeIn 0.4s ease",maxWidth:500,margin:"0 auto",position:"relative",boxShadow:"0 0 80px rgba(0,0,0,0.5)"}}>
    ...
    <div style={{position:"fixed",top:12,left:"50%",transform:"translateX(-50%)",zIndex:9999,display:"flex",flexDirection:"column",gap:8,width:"92%",maxWidth:460,pointerEvents:"none"}}>
      {toasts.map(t=>(
        <div key={t.id} onClick={()=>{openChat(t.contact);setToasts(p=>p.filter(x=>x.id!==t.id));}} style={{background:T.card,borderRadius:16,padding:"11px 15px",display:"flex",alignItems:"center",gap:11,boxShadow:"0 8px 32px rgba(0,0,0,0.5)",border:`1px solid ${T.border}`,animation:"slideDown 0.3s ease",cursor:"pointer",pointerEvents:"all"}}>
          <Av name={t.name} size={34} />
          <div style={{flex:1,overflow:"hidden"}}><div style={{fontWeight:700,fontSize:12,color:T.blue}}>{t.name}</div><div style={{fontSize:13,color:T.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",marginTop:1}}>{t.text}</div></div>
    ...
    {/* Image Preview */}
    {previewImg&&<div onClick={()=>setPreviewImg(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.97)",zIndex:9998,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:14,animation:"fadeIn 0.2s ease"}}>
      <img src={previewImg} alt="p" style={{maxWidth:"95vw",maxHeight:"82vh",borderRadius:18,boxShadow:"0 20px 60px rgba(0,0,0,0.8)"}} />
      <div style={{display:"flex",gap:12}}>
        <a href={previewImg} download="khan-chats-image.jpg" style={{padding:"10px 22px",background:T.grad,borderRadius:14,color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",textDecoration:"none",boxShadow:T.shadow}}>⬇ Download</a>
        <div onClick={()=>setPreviewImg(null)} style={{padding:"10px 22px",background:T.card,borderRadius:14,color:T.text,fontWeight:700,fontSize:13,cursor:"pointer",border:`1px solid ${T.border}`}}>Close ✕</div>
      </div>
    </div>}
    ...
    {viewS&&<div onClick={()=>setViewS(null)} style={{position:"fixed",inset:0,background:"#000",zIndex:10000,display:"flex",flexDirection:"column",animation:"fadeIn 0.2s ease"}}>
      <div style={{padding:"18px 20px",display:"flex",alignItems:"center",gap:13,background:"rgba(0,0,0,0.7)"}}>
        <Av name={viewS.name} size={44} />
        <div><div style={{fontWeight:700,color:"#fff",fontSize:15}}>{viewS.name}</div><div style={{fontSize:11,color:"rgba(255,255,255,0.5)",marginTop:2}}>{tAgo(viewS.timestamp)}</div></div>
        <span style={{marginLeft:"auto",fontSize:22,color:"rgba(255,255,255,0.6)",cursor:"pointer"}}>✕</span>
      </div>
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:22}}>
        {viewS.image&&<img src={viewS.image} alt="s" style={{maxWidth:"100%",maxHeight:"70vh",borderRadius:18}} />}
    ...
    {showInvite&&<Modal onClose={()=>setShowInvite(false)}>
      <Card style={{padding:30,textAlign:"center"}}>
        <div style={{width:66,height:66,borderRadius:20,background:T.grad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,margin:"0 auto 14px",boxShadow:T.shadow}}>🔗</div>
        <h3 style={{color:T.text,fontWeight:800,fontSize:19,marginBottom:8,fontFamily:"'Poppins',sans-serif"}}>Invite Friends</h3>
        <p style={{color:T.muted,fontSize:13,marginBottom:18}}>Share this link to invite friends</p>
        <div style={{background:T.card2,borderRadius:12,padding:"11px 14px",fontSize:11,color:T.blue,wordBreak:"break-all",marginBottom:18,border:`1px solid ${T.border}`,lineHeight:1.6}}>{invL}</div>
        <Btn onClick={copyLink} style={{marginBottom:10}}>{copied?"✅ Copied!":"📋 Copy Link"}</Btn>
        <div onClick={()=>setShowInvite(false)} style={{padding:"10px",color:T.muted,cursor:"pointer",fontSize:13}}>Close</div>
      </Card>
    </Modal>}

    {policy&&<Modal onClose={()=>setPolicy(null)}>
      <Card style={{padding:26,maxHeight:"75vh",overflowY:"auto"}}>
        <div style={{fontWeight:800,fontSize:17,color:T.text,marginBottom:12}}>{policy==="privacy"?"🔒 Privacy Policy":"📋 Terms"}</div>
        <div style={{color:T.mutedL,fontSize:13,lineHeight:1.9}}>{policy==="privacy"?<><p>Independent platform committed to privacy.</p><p><strong style={{color:T.text}}>Data:</strong> Email, name, photo, messages.</p><p><strong style={{color:T.text}}>Security:</strong> Firebase.</p></>:<><p>By using Khan Chats you agree.</p><p><strong style={{color:T.text}}>Disclaimer:</strong> Not affiliated with WhatsApp or Meta.</p></>}</div>
        <Btn onClick={()=>setPolicy(null)} style={{marginTop:16}}>Close</Btn>
      </Card>
    </Modal>}
      ...
      <div style={{display:"flex",flexDirection:"column",height:"100%",background:T.bg}}>
        ...
        <div style={{display:"flex",alignItems:"center",padding:"12px 14px",background:T.card,gap:11,borderBottom:`1px solid ${T.border}`,boxShadow:"0 2px 14px rgba(0,0,0,0.3)",flexShrink:0,zIndex:10}}>
          ...
          <div style={{flex:1,overflow:"hidden",cursor:"pointer"}}>
            ...
            <div style={{fontSize:10,fontWeight:600,marginTop:1,transition:"all 0.3s",color:isTyping?"#A78BFA":T.blue}}>
              {isTyping?<span style={{animation:"fadeIn 0.3s ease"}}>typing...</span>:"● Online"}
            </div>
          </div>
          <div style={{display:"flex",gap:6,flexShrink:0}}>
            {[["📞",()=>startCall("audio")],["📹",()=>startCall("video")]].map(([icon,fn])=>(
              <div key={icon} onClick={fn} style={{width:36,height:36,borderRadius:11,background:T.card2,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:16,border:`1px solid ${T.border}`,transition:"all 0.15s"}}
        ...
        {/* Messages Area */}
        <div style={{flex:1,overflowY:"auto",padding:"14px 12px",display:"flex",flexDirection:"column",gap:2,position:"relative",...chatBgStyle()}}
          onClick={()=>{setMsgMenu(null);setShowEmoji(false);}}>

          {msgs.length===0&&<div style={{textAlign:"center",margin:"auto",color:T.muted,padding:40}}>
            <div style={{fontSize:54,marginBottom:14,opacity:0.3}}>👋</div>
            <div style={{fontSize:16,fontWeight:700,color:T.text,fontFamily:"'Poppins',sans-serif"}}>Say hello!</div>
            <div style={{fontSize:12,marginTop:8,lineHeight:1.6}}>Start your conversation with<br/><strong style={{color:T.blue}}>{activeChat.name}</strong></div>
          </div>}
          ...
          {msgs.map((msg,i)=>{
            ...
            return(
              <div key={i}>
                {showDay&&<div style={{textAlign:"center",margin:"10px 0 6px",animation:"fadeIn 0.3s ease"}}>
                  <span style={{fontSize:11,color:T.mutedL,fontWeight:600,background:T.card2,padding:"4px 14px",borderRadius:20,border:`1px solid ${T.border}`}}>{dayLabel(msg.timestamp)}</span>
                </div>}

                <div style={{display:"flex",justifyContent:isMine?"flex-end":"flex-start",marginBottom:showAv?6:1,animation:"msgIn 0.2s ease",position:"relative"}}
                  onContextMenu={e=>{e.preventDefault();if(!isDeleted)setMsgMenu({id:i,msgId:Object.keys(msgs)[i]||i,isMine,text:msg.text,x:e.clientX,y:e.clientY,msg});}}>

                  {/* Avatar */}
                  {!isMine&&<div style={{width:28,height:28,marginRight:7,flexShrink:0,alignSelf:"flex-end",marginBottom:2}}>
                    {showAv&&<div style={{width:28,height:28,borderRadius:9,background:cfn(msg.senderName),display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:9,color:"#fff"}}>{gi(msg.senderName)}</div>}
                  </div>}

                  <div style={{maxWidth:"75%",display:"flex",flexDirection:"column",alignItems:isMine?"flex-end":"flex-start"}}>

                    {/* Reply preview */}
                    {msg.replyTo&&!isDeleted&&<div style={{padding:"7px 12px",background:isMine?"rgba(255,255,255,0.08)":"rgba(79,142,247,0.08)",borderRadius:"12px 12px 0 0",marginBottom:-3,borderLeft:`3px solid ${T.blue}`,maxWidth:"100%",overflow:"hidden"}}>
                      <div style={{fontSize:10,color:T.blue,fontWeight:700,marginBottom:2}}>{msg.replyTo.senderName}</div>
                      <div style={{fontSize:12,color:T.mutedL,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{msg.replyTo.text}</div>
                    </div>}

                    {/* Message Bubble */}
                    <div style={{
                      padding:msg.image?"6px 6px 8px":(isDeleted?"10px 14px 8px":"11px 15px 9px"),
                      background:isDeleted?"transparent":(isMine?T.gradS:T.card2),
                      borderRadius:isMine?(msg.replyTo?"0 0 5px 18px":"20px 20px 5px 20px"):(msg.replyTo?"0 0 18px 5px":"20px 20px 20px 5px"),
                      boxShadow:isDeleted?"none":(isMine?"0 3px 14px rgba(30,58,138,0.45)":"0 2px 10px rgba(0,0,0,0.3)"),
                      border:isDeleted?`1px dashed ${T.border}`:(isMine?"none":`1px solid ${T.border}`),
                      cursor:"context-menu",
                      transition:"all 0.15s"
                    }}
                      onLongPress={()=>!isDeleted&&setMsgMenu({id:i,msgId:i,isMine,text:msg.text})}>

                      {!isMine&&showAv&&!isDeleted&&<div style={{fontSize:10,color:cfn(msg.senderName),fontWeight:700,marginBottom:4}}>{msg.senderName}</div>}

                      {isDeleted?(
                        <div style={{fontSize:13,color:T.muted,fontStyle:"italic",display:"flex",alignItems:"center",gap:6}}>
                        ...
                        <>
                          {msg.image&&(
                            <div style={{position:"relative"}}>
                              <img src={msg.image} alt="s" onClick={e=>{e.stopPropagation();setPreviewImg(msg.image);}}
                                style={{maxWidth:220,maxHeight:220,borderRadius:msg.replyTo?"0 0 12px 12px":14,display:"block",cursor:"pointer",objectFit:"cover",transition:"opacity 0.2s"}}
                                onMouseEnter={e=>e.target.style.opacity="0.9"}
                                onMouseLeave={e=>e.target.style.opacity="1"} />
                              <div style={{position:"absolute",bottom:8,right:8,background:"rgba(0,0,0,0.6)",borderRadius:10,padding:"2px 8px",display:"flex",alignItems:"center",gap:4}}>
                                <span style={{fontSize:10,color:"rgba(255,255,255,0.85)"}}>{ft(msg.timestamp)}</span>
                          ...
                          {msg.voice&&(
                            <div style={{display:"flex",alignItems:"center",gap:10,padding:"4px 0",minWidth:160}}>
                              <div style={{width:34,height:34,borderRadius:"50%",background:isMine?"rgba(255,255,255,0.15)":T.card3,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,cursor:"pointer",flexShrink:0}}>▶️</div>
                              <div style={{flex:1}}>
                                <div style={{height:3,background:isMine?"rgba(255,255,255,0.25)":T.border,borderRadius:2,marginBottom:4,position:"relative",overflow:"hidden"}}>
                                  <div style={{width:"45%",height:"100%",background:isMine?"rgba(255,255,255,0.6)":T.blue,borderRadius:2}} />
                                </div>
                                <div style={{fontSize:10,color:isMine?"rgba(255,255,255,0.5)":T.muted}}>0:07</div>
                              </div>
                      ...
                      {!msg.image&&!isDeleted&&<div style={{display:"flex",justifyContent:"flex-end",alignItems:"center",gap:3,marginTop:4}}>
                        ...
                        {isMine&&(
                          <span style={{fontSize:12,color:isLast?"#60A5FA":"rgba(255,255,255,0.5)",letterSpacing:-1}}>
                            {isLast?"✓✓":"✓✓"}
                          </span>
                        )}
                      </div>}
                  ...
                  <div style={{position:"absolute",[isMine?"left":"right"]:-32,top:"50%",transform:"translateY(-50%)",opacity:0,transition:"opacity 0.2s"}}
                    onMouseEnter={e=>e.currentTarget.style.opacity="1"}
                    onMouseLeave={e=>e.currentTarget.style.opacity="0"}>
                    {!isDeleted&&<div onClick={e=>{e.stopPropagation();setReplyTo({text:msg.text||"📷 Photo",senderName:msg.senderName,msgId:i});}} style={{width:26,height:26,borderRadius:8,background:T.card,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:12,border:`1px solid ${T.border}`}}>↩</div>}
                  </div>
                </div>
              </div>
            );
          ...
          {isTyping&&<div style={{display:"flex",alignItems:"flex-end",gap:8,marginTop:4,animation:"msgIn 0.3s ease"}}>
            <div style={{width:28,height:28,borderRadius:9,background:cfn(activeChat.name),display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:9,color:"#fff",flexShrink:0}}>{gi(activeChat.name)}</div>
            <div style={{padding:"11px 16px",background:T.card2,borderRadius:"18px 18px 18px 5px",border:`1px solid ${T.border}`,display:"flex",gap:5,alignItems:"center"}}>
              {[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:"#A78BFA",animation:`dot 1.4s ${i*0.2}s infinite`}} />)}
            </div>
          </div>}
          <div ref={endRef} />
        </div>
        ...
        {msgMenu&&<div ref={msgMenuRef} style={{position:"fixed",top:Math.min(msgMenu.y||300,window.innerHeight-220),left:Math.min(Math.max(msgMenu.x||100,10),window.innerWidth-180),background:T.card,borderRadius:16,padding:6,border:`1px solid ${T.border}`,boxShadow:"0 8px 32px rgba(0,0,0,0.6)",zIndex:500,minWidth:170,animation:"menuIn 0.2s ease"}}>
          {[
            ["↩️ Reply",()=>{setReplyTo({text:msgMenu.msg?.text||"📷 Photo",senderName:msgMenu.msg?.senderName||"",msgId:msgMenu.msgId});setMsgMenu(null);}],
            ...
            <div key={label} onClick={fn} style={{padding:"11px 16px",borderRadius:11,cursor:"pointer",fontSize:13,fontWeight:600,color:label.includes("Delete")||label.includes("🚫")?"#EF4444":T.text,transition:"background 0.15s"}}
        ...
        {/* Reply Preview Bar */}
        {replyTo&&<div style={{padding:"10px 14px",background:T.card2,borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10,animation:"slideDown 0.2s ease",flexShrink:0}}>
          <div style={{flex:1,borderLeft:`3px solid ${T.blue}`,paddingLeft:10}}>
            <div style={{fontSize:11,color:T.blue,fontWeight:700,marginBottom:2}}>{replyTo.senderName}</div>
            <div style={{fontSize:12,color:T.mutedL,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{replyTo.text}</div>
          </div>
          <div onClick={()=>setReplyTo(null)} style={{color:T.muted,cursor:"pointer",fontSize:18,padding:4}}>✕</div>
        </div>}

        {/* Emoji Picker */}
        {showEmoji&&<div style={{background:T.card,borderTop:`1px solid ${T.border}`,padding:12,animation:"slideUp 0.2s ease",flexShrink:0}}>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,justifyContent:"center"}}>
            {EMOJIS.map(e=>(
              <div key={e} onClick={()=>{setInp(p=>p+e);}} style={{fontSize:22,cursor:"pointer",padding:"5px 7px",borderRadius:8,transition:"background 0.15s",userSelect:"none"}}
        ...
        <div style={{padding:"10px 11px",background:T.card,borderTop:`1px solid ${T.border}`,flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"flex-end",gap:7}}>
            ...
            {/* Emoji */}
            <div onClick={()=>setShowEmoji(p=>!p)} style={{width:38,height:38,borderRadius:12,background:showEmoji?T.grad:T.card2,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:18,border:`1px solid ${T.border}`,flexShrink:0,transition:"all 0.2s",marginBottom:1}}>
              😊
            </div>

            {/* Text Input */}
            <div style={{display:"flex",alignItems:"flex-end",background:T.card2,borderRadius:20,flex:1,padding:"8px 12px",gap:8,border:`1.5px solid ${T.border}`,minHeight:42,transition:"border 0.2s"}}
              onFocus={()=>{}}
              style={{display:"flex",alignItems:"center",background:T.card2,borderRadius:20,flex:1,padding:"8px 12px",gap:8,border:`1.5px solid ${T.border}`,minHeight:42}}>
              <textarea value={inp} onChange={e=>handleTyping(e.target.value)}
                onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMsg();}}}
                placeholder="Message..."
                rows={1}
                style={{flex:1,background:"none",border:"none",outline:"none",color:T.text,fontSize:14,fontFamily:"'Inter',sans-serif",resize:"none",lineHeight:1.5,maxHeight:100,overflowY:"auto"}} />
            ...
            {/* Send/Mic */}
            {inp.trim()?(
              <div onClick={()=>sendMsg()} style={{width:42,height:42,borderRadius:14,background:T.grad,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:17,boxShadow:T.shadow,transition:"all 0.2s",flexShrink:0}}
              ...
              <div onClick={toggleRecording} style={{width:42,height:42,borderRadius:14,background:isRecording?"#EF4444":T.card2,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:18,border:`1px solid ${T.border}`,flexShrink:0,transition:"all 0.2s",animation:isRecording?"recPulse 1s infinite":"none"}}>
                {isRecording?<><span style={{fontSize:11,color:"#fff",fontWeight:700}}>{fD(recTime)}</span></>:"🎤"}
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
        ...
        <div style={{padding:"14px 16px 11px",background:T.card,borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:11}}>
            <div style={{width:38,height:38,borderRadius:13,background:T.grad,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:15,color:"#fff",overflow:"hidden",cursor:"pointer",boxShadow:T.shadow,flexShrink:0}}
              onClick={()=>{setShowSett(true);setSTab("profile");}}>
              {pic?<img src={pic} alt="p" style={{width:38,height:38,objectFit:"cover"}} />:gi(user?.displayName||user?.email)}
            </div>
            <div>
              <div style={{fontWeight:800,fontSize:17,color:T.text,letterSpacing:"-0.4px",fontFamily:"'Poppins',sans-serif"}}>Khan Chats</div>
              <div style={{fontSize:9,color:T.blue,fontWeight:600}}>● Active</div>
            </div>
          </div>
        ...
        {/* Search */}
        {showSearch&&<div style={{padding:"9px 13px",background:T.card2,borderBottom:`1px solid ${T.border}`,animation:"slideDown 0.2s ease",flexShrink:0}}>
        ...
        <div style={{flex:1,overflowY:"auto",paddingBottom:68}}>
          ...
          {view==="messages"&&<div style={{animation:"fadeIn 0.25s ease"}}>
            {showNew&&<div style={{padding:"11px 13px",background:T.card2,borderBottom:`1px solid ${T.border}`,animation:"slideDown 0.2s ease"}}>
              <Inp value={nEmail} onChange={e=>setNEmail(e.target.value)} placeholder="Enter friend's email..." onKeyDown={e=>e.key==="Enter"&&startChat()} style={{marginBottom:9}} />
              {nEmailErr&&<div style={{color:"#EF4444",fontSize:12,marginBottom:8,padding:"6px 11px",background:"rgba(239,68,68,0.08)",borderRadius:9}}>{nEmailErr}</div>}
              <div style={{display:"flex",gap:8}}>
                <Btn onClick={startChat} style={{flex:1,padding:"10px",borderRadius:12,fontSize:13}}>Start Chat</Btn>
                <Btn onClick={()=>{setShowNew(false);setNEmailErr("");}} v="ghost" style={{padding:"10px 14px",borderRadius:12}}>✕</Btn>
              </div>
            </div>}

            {!searchQ&&filtC.length===0&&lkC.length===0?(
              <div style={{padding:"24px 16px",animation:"fadeIn 0.3s ease"}}>
                <div style={{background:T.grad,borderRadius:22,padding:22,marginBottom:16,position:"relative",overflow:"hidden",boxShadow:T.shadowL}}>
                  <div style={{position:"absolute",top:-15,right:-15,width:90,height:90,borderRadius:"50%",background:"rgba(255,255,255,0.05)"}} />
                  <div style={{position:"relative"}}>
                    <div style={{fontSize:32,marginBottom:8}}>👋</div>
                    <div style={{fontWeight:800,fontSize:18,color:"#fff",fontFamily:"'Poppins',sans-serif",marginBottom:5}}>Welcome, {user?.displayName?.split(" ")[0]}!</div>
                    <div style={{fontSize:12,color:"rgba(255,255,255,0.7)",lineHeight:1.6,marginBottom:16}}>Start connecting on Khan Chats</div>
                    <div style={{display:"flex",gap:9}}>
                      <Btn onClick={()=>setShowNew(true)} style={{background:"rgba(255,255,255,0.18)",borderRadius:12,padding:"10px 18px",fontSize:12,boxShadow:"none",border:"1px solid rgba(255,255,255,0.2)"}}>✏️ New Chat</Btn>
                      <Btn onClick={genInvite} style={{background:"rgba(255,255,255,0.12)",borderRadius:12,padding:"10px 18px",fontSize:12,boxShadow:"none",border:"1px solid rgba(255,255,255,0.15)"}}>🔗 Invite</Btn>
                    </div>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                  {[["💬","Messages","Start a chat",()=>setShowNew(true)],["✨","Updates","Share status",()=>setView("updates")],["📞","Calls","Call history",()=>setView("calls")],["🤖","Khan AI","AI assistant",()=>{setShowSett(true);setSTab("ai");}]].map(([icon,title,desc,fn])=>(
                    <div key={title} onClick={fn} style={{background:T.card,borderRadius:16,padding:16,cursor:"pointer",border:`1px solid ${T.border}`,transition:"all 0.2s",boxShadow:T.cardShadow}}
                      onMouseEnter={e=>e.currentTarget.style.background=T.card2}
              ...
              <>
                {/* Recent Avatars */}
                {!searchQ&&recentAct.length>0&&<div style={{padding:"11px 14px 0"}}>
                  <div style={{fontSize:9,color:T.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:1.5,marginBottom:9}}>Recent</div>
                  <div style={{display:"flex",gap:11,overflowX:"auto",paddingBottom:10}}>
                    {recentAct.map(c=>(
                      <div key={c.chatId} onClick={()=>handleChatClick(c)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,cursor:"pointer",flexShrink:0}}>
                        <div style={{position:"relative"}}>
                          <div style={{width:50,height:50,borderRadius:16,background:cfn(c.name),display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:17,color:"#fff"}}>{gi(c.name)}</div>
                          {(unread[c.chatId]||0)>0&&<div style={{position:"absolute",top:-3,right:-3,width:17,height:17,background:T.gradD,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:800,color:"#fff",animation:"badgePop 0.3s ease"}}>{(unread[c.chatId]||0)>9?"9+":unread[c.chatId]}</div>}
                        </div>
                        <div style={{fontSize:10,color:T.mutedL,fontWeight:600,maxWidth:56,textAlign:"center",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.name.split(" ")[0]}</div>
                      </div>
                    ))}
                  </div>
                </div>}
                ...
                {filtC.map(([chatId,contact],idx)=>(
                  <div key={chatId} onClick={()=>handleChatClick(contact)}
                    style={{display:"flex",alignItems:"center",padding:"12px 16px",cursor:"pointer",gap:12,background:activeChat?.chatId===chatId?T.card2:"transparent",borderBottom:`1px solid ${T.border}`,transition:"all 0.15s ease",animation:`slideUp 0.3s ${Math.min(idx*0.04,0.3)}s ease both`}}
                    ...
                    <div style={{flex:1,overflow:"hidden"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                        ...
                        <span style={{fontSize:10,color:(unread[chatId]||0)>0?T.blue:T.muted,fontWeight:500,flexShrink:0}}>{tAgo(contact.lastTime)}</span>
                      </div>
                ...
                {lkC.length>0&&<>
                  <div onClick={()=>setShowLocked(p=>!p)} style={{display:"flex",alignItems:"center",padding:"11px 16px",cursor:"pointer",background:T.card2,borderBottom:`1px solid ${T.border}`,gap:10}}>
                    <div style={{width:38,height:38,borderRadius:12,background:"rgba(239,68,68,0.07)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,border:"1px solid rgba(239,68,68,0.12)"}}>🔒</div>
                    <span style={{fontWeight:700,fontSize:13,color:T.muted,flex:1}}>Locked Chats ({lkC.length})</span>
                    <span style={{color:T.muted,fontSize:12}}>{showLocked?"▲":"▼"}</span>
                  </div>
                  {showLocked&&lkC.map(([chatId],idx)=>(
                    <div key={chatId} onClick={()=>handleChatClick(contacts[chatId])} style={{display:"flex",alignItems:"center",padding:"12px 16px",cursor:"pointer",gap:12,background:T.bg,borderBottom:`1px solid ${T.border}`,animation:`slideUp 0.2s ${idx*0.04}s ease both`}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:12,color:T.muted,marginTop:2}}>Tap to unlock</div>
                      </div>
                    </div>
                  ))}
                </>}

                {searchQ&&filtC.length===0&&<div style={{padding:46,textAlign:"center",color:T.muted}}>
                  <div style={{fontSize:46,marginBottom:10,opacity:0.25}}>🔍</div>
                  <div style={{fontWeight:700,fontSize:14,color:T.text}}>No results found</div>
                  <div style={{fontSize:12,marginTop:5}}>Try different keywords</div>
                </div>}
              </>
            )}
          </div>}
          ...
          {view==="updates"&&<div style={{animation:"fadeIn 0.25s ease"}}>
            <div style={{padding:"12px 14px",borderBottom:`1px solid ${T.border}`}}>
              <div onClick={()=>setShowAddS(p=>!p)} style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer",padding:"2px 0"}}>
                <div style={{width:54,height:54,borderRadius:18,background:myS.length>0?cfn(user?.displayName):T.card2,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:18,color:"#fff",border:myS.length>0?`2.5px solid ${T.blue}`:`2.5px dashed ${T.border}`,flexShrink:0,overflow:"hidden"}}>
                  {pic?<img src={pic} alt="p" style={{width:54,height:54,objectFit:"cover"}} />:gi(user?.displayName)}
                </div>
                <div style={{flex:1}}>
                  <div style={{fontWeight:700,fontSize:14,color:T.text}}>My Update</div>
                  <div style={{fontSize:11,color:T.muted,marginTop:2}}>{myS.length>0?tAgo(myS[0].timestamp):"Share what's on your mind"}</div>
                </div>
              ...
              {showAddS&&<div style={{marginTop:11,padding:13,background:T.card2,borderRadius:15,border:`1px solid ${T.border}`,animation:"slideDown 0.2s ease"}}>
                ...
                <div style={{display:"flex",gap:7}}>
                  <Btn onClick={()=>postS()} style={{flex:1,padding:"10px",borderRadius:11,fontSize:12}}>Share ✓</Btn>
                  <Btn onClick={()=>sFRef.current?.click()} v="ghost" style={{padding:"10px 13px",borderRadius:11}}>📷</Btn>
                  <Btn onClick={()=>setShowAddS(false)} v="ghost" style={{padding:"10px 13px",borderRadius:11}}>✕</Btn>
                </div>
            ...
            {othS.map((s,i)=>(
              <div key={i} onClick={()=>setViewS(s)} style={{display:"flex",alignItems:"center",padding:"12px 16px",gap:12,cursor:"pointer",borderBottom:`1px solid ${T.border}`,transition:"background 0.15s",animation:`slideUp 0.3s ${i*0.05}s ease both`}}
                onMouseEnter={e=>e.currentTarget.style.background=T.card}
                onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <div style={{width:54,height:54,borderRadius:18,background:cfn(s.name),display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:18,color:"#fff",border:`2.5px solid ${T.blue}`,flexShrink:0}}>{gi(s.name)}</div>
                <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14,color:T.text}}>{s.name}</div><div style={{fontSize:11,color:T.muted,marginTop:2}}>{tAgo(s.timestamp)}</div></div>
                {s.image&&<div style={{width:44,height:44,borderRadius:12,overflow:"hidden",flexShrink:0}}><img src={s.image} alt="s" style={{width:"100%",height:"100%",objectFit:"cover"}} /></div>}
              </div>
            ))}
            {statuses.length===0&&<div style={{padding:50,textAlign:"center",color:T.muted}}>
              <div style={{fontSize:52,marginBottom:12,opacity:0.25}}>✨</div>
              <div style={{fontWeight:700,fontSize:14,color:T.text,fontFamily:"'Poppins',sans-serif"}}>No updates yet</div>
            </div>}
          </div>}
          ...
          {view==="calls"&&<div style={{animation:"fadeIn 0.25s ease"}}>
            <div style={{display:"flex",padding:"2px 8px",background:T.card,borderBottom:`1px solid ${T.border}`,overflowX:"auto",gap:2}}>
              {["all","missed","incoming","outgoing"].map(f=>(
                <div key={f} onClick={()=>setCallF(f)} style={{padding:"9px 11px",cursor:"pointer",fontSize:10,fontWeight:700,whiteSpace:"nowrap",color:callF===f?T.blue:"rgba(74,85,104,0.65)",borderBottom:callF===f?`2.5px solid ${T.blue}`:"2.5px solid transparent",transition:"all 0.2s",letterSpacing:0.3}}>
                  {f==="missed"?"📵 Missed":f==="incoming"?"📞 In":f==="outgoing"?"📲 Out":"All"}
                </div>
              ))}
            </div>
            {filtCalls.length===0?<div style={{padding:50,textAlign:"center",color:T.muted}}><div style={{fontSize:52,marginBottom:12,opacity:0.25}}>📞</div><div style={{fontWeight:700,fontSize:14,color:T.text,fontFamily:"'Poppins',sans-serif"}}>No calls yet</div></div>
            :filtCalls.map((call,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",padding:"12px 16px",gap:12,borderBottom:`1px solid ${T.border}`,animation:`slideUp 0.3s ${Math.min(i*0.04,0.3)}s ease both`}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,fontWeight:600,marginTop:2,color:call.status==="missed"?"#EF4444":call.direction==="incoming"?T.blue:T.purple}}>
                    {call.status==="missed"?"📵 Missed":call.direction==="incoming"?`📞 Incoming · ${call.type}`:`📲 Outgoing · ${call.type}`}
                  </div>
                  <div style={{fontSize:10,color:T.muted,marginTop:1}}>{new Date(call.timestamp).toLocaleString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}</div>
                </div>
        ...
        <div style={{position:"absolute",bottom:0,left:0,right:0,display:"flex",background:T.card,borderTop:`1px solid ${T.border}`,zIndex:50,boxShadow:"0 -4px 20px rgba(0,0,0,0.3)"}}>
          {[["home","🏠","Home",()=>{setView("messages");setShowNew(false);}],["compose","✏️","New Chat",()=>{setView("messages");setShowNew(true);}],["updates","✨","Updates",()=>setView("updates")],["settings","⚙️","Settings",()=>setShowSett(true)]].map(([id,icon,label,fn])=>{
            const isActive=(id==="home"&&view==="messages"&&!showNew&&!showSett)||(id==="compose"&&showNew)||(id==="updates"&&view==="updates")||(id==="settings"&&showSett);
            return(
              <div key={id} onClick={fn} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"9px 4px 10px",cursor:"pointer",position:"relative",transition:"all 0.2s",minWidth:0}}
                onMouseDown={e=>e.currentTarget.style.opacity="0.65"}
                onMouseUp={e=>e.currentTarget.style.opacity="1"}
                onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
                <div style={{width:36,height:36,borderRadius:12,background:isActive?T.grad:"transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,marginBottom:3,transition:"all 0.25s",boxShadow:isActive?T.shadow:"none",flexShrink:0}}>
                  {icon}
                </div>
        ...
        {/* FAB */}
        {view==="messages"&&!showNew&&<div onClick={()=>setShowNew(true)} style={{position:"absolute",bottom:76,right:16,width:52,height:52,borderRadius:17,background:T.grad,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:22,boxShadow:T.shadowL,transition:"all 0.2s",zIndex:49}}
          onMouseDown={e=>e.currentTarget.style.transform="scale(0.92)"}
          onMouseUp={e=>e.currentTarget.style.transform="scale(1)"}
          onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>
          ✏️
        ...
        {/* Footer */}
        <div style={{position:"absolute",bottom:62,left:0,right:0,textAlign:"center",pointerEvents:"none",zIndex:40}}>
          <div style={{fontSize:8,color:"rgba(74,85,104,0.35)"}}>Khan Chats · Independent · Not affiliated with WhatsApp or Meta</div>
        </div>
      </div>
    )}
  </div>
