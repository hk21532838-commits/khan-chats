...
const GEMINI_KEY="AIzaSyBD7nr5CaWz4VNW0MaZQdliPb4YKKdYmrg";
...
const sendMsg=async(imgData=null)=>{
  ...
  const msg={text:imgData?"":inp.trim(),image:imgData||null,senderUid:user.uid,senderName:user.displayName||user.email,timestamp:Date.now(),status:"sent",...(replyTo?{replyTo:{text:replyTo.text||"📷 Photo",senderName:replyTo.senderName}}:{})};
...
const startCall=async type=>{
  if(!activeChat)return;setCallType(type);setInCall(true);setCallStart(Date.now());
  try{
...
{[["Display Name",newName,setNewName,"Your name"],["Username",uname,setUname,"@username"],["Bio",bio,setBio,"About you"]].map(([l,v,fn,ph])=>(<div key={l} style={{marginBottom:12}}><div style={{fontSize:11,color:T.mutedL,fontWeight:600,marginBottom:5}}>{l}</div><Inp value={v} onChange={e=>fn(e.target.value)} placeholder={ph} /></div>))}
...
{Object.entries(contacts).length===0?<div style={{color:T.muted,fontSize:13,textAlign:"center",padding:14}}>No contacts yet</div>:Object.entries(contacts).map(([chatId,c])=>(<div key={chatId} style={{display:"flex",alignItems:"center",padding:"10px 12px",background:T.card2,borderRadius:13,marginBottom:8,gap:11,border:`1px solid ${T.border}`}}><Av name={c.name} size={38} /><div style={{flex:1}}><div style={{fontWeight:700,fontSize:13,color:T.text}}>{c.name}</div><div style={{fontSize:11,color:locks[chatId]?"#EF4444":T.muted,fontWeight:600}}>{locks[chatId]?"🔒 Locked":"🔓 Unlocked"}</div></div>{locks[chatId]?<div onClick={()=>{if(window.confirm("Remove lock?"))removeLock(chatId);}} style={{padding:"6px 12px",background:"rgba(239,68,68,0.1)",borderRadius:10,color:"#EF4444",fontSize:12,fontWeight:700,cursor:"pointer"}}>Remove</div>:<div onClick={()=>{setLModal(chatId);setLPin("");setLErr("");}} style={{padding:"6px 14px",background:T.grad,borderRadius:10,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>🔒 Lock</div>}</div>))}
</Card>
...
<div style={{position:"absolute",top:-15,right:-15,width:80,height:80,borderRadius:"50%",background:"rgba(255,255,255,0.05)"}} />
...
<div style={{width:52,height:52,borderRadius:17,background:myS.length>0?cfn(user?.displayName):T.card2,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:18,color:"#fff",border:myS.length>0?`2.5px solid ${T.blue}`:`2.5px dashed ${T.border}`,flexShrink:0,overflow:"hidden"}}>
{pic?<img src={pic} alt="p" style={{width:52,height:52,objectFit:"cover"}} />:gi(user?.displayName)}
</div>
...
{showAddS&&<div style={{marginTop:11,padding:13,background:T.card2,borderRadius:15,border:`1px solid ${T.border}`,animation:"slideDown 0.2s ease"}}><Inp value={sText} onChange={e=>setSText(e.target.value)} placeholder="What's on your mind?" style={{marginBottom:9}} /><div style={{display:"flex",gap:7}}><Btn onClick={()=>postS()} style={{flex:1,padding:"10px",borderRadius:11,fontSize:12}}>Share ✓</Btn><Btn onClick={()=>sFRef.current?.click()} v="ghost" style={{padding:"10px 13px",borderRadius:11}}>📷</Btn><Btn onClick={()=>setShowAddS(false)} v="ghost" style={{padding:"10px 13px",borderRadius:11}}>✕</Btn></div><input type="file" accept="image/*" ref={sFRef} onChange={handleSImg} style={{display:"none"}} /></div>}
</div>
