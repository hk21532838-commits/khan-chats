...
export default function App() {
  ...
  const register = async () => {
    if (!displayName.trim()) { setAuthError("Naam likhein"); return; }
    setAuthLoading(true); setAuthError("");
    try {
      const cred = await createUserWithEmailAndPassword(auth, email, password);
      await updateProfile(cred.user, { displayName: displayName.trim() });
    } catch (e) {
      setAuthError(e.message.includes("email-already") ? "Ye email registered hai" : e.message.includes("weak") ? "Password 6+ characters ka hona chahiye" : "Koi masla hua");
    }
  ...
  const login = async () => {
    ...
    catch { setAuthError("Email ya password galat hai"); }
    setAuthLoading(false);
  };
  ...
  const startChat = async () => {
    setNewChatError("");
    if (!newChatEmail.trim()) { setNewChatError("Email likhein"); return; }
    if (newChatEmail.trim() === user.email) { setNewChatError("Apne aap ko message nahi kar sakte!"); return; }
    const snap = await get(ref(db, "users"));
    const found = Object.values(snap.val() || {}).find(u => u.email === newChatEmail.trim());
    if (!found) { setNewChatError("User nahi mila. Pehle invite karein!"); return; }
  ...
  const handleImage = (e) => {
    ...
    if (!file) return;
    if (file.size > 500000) { alert("Photo 500KB se chhoti honi chahiye"); return; }
  ...
  const startCall = async (type) => {
    ...
    try {
      ...
      onValue(ref(db, `calls/${activeChat.chatId}/answer`), async (snap) => {
        if (snap.val() && pc.signalingState !== "stable") {
    ...
    } catch (err) { alert("Microphone/Camera access nahi mila: " + err.message); setInCall(false); }
  };
  ...
  useEffect(() => {
    ...
    onValue(callRef, async (snap) => {
      ...
      if (data && data.caller !== user.uid && data.offer && !inCall) {
        if (window.confirm(`${data.callerName} ka ${data.type === "video" ? "Video" : "Audio"} call aa raha hai! Receive karein?`)) {
          ...
          } catch (err) { alert("Call receive nahi hua: " + err.message); setInCall(false); }
        }
      }
    });
  }, [user, activeChat]);
    ...
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", background:"#111b21", fontFamily:"'Segoe UI',sans-serif" }}>
      <div style={{ width:"100%", maxWidth:380, padding:"0 24px" }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          ...
          <p style={{ color:"#8696a0", fontSize:14, margin:"8px 0 0" }}>Apne doston se baat karein</p>
        </div>
        ...
        {screen === "register" && (
          <input value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder="Apna naam likhein"
        ...
        <div onClick={screen==="login"?login:register}
          ...
          {authLoading ? "Please wait..." : screen==="login" ? "Login Karein" : "Account Banayein"}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ display:"flex", height:"100vh", fontFamily:"'Segoe UI',sans-serif", background:"#111b21", color:"#e9edef", overflow:"hidden", position:"relative" }}>
      ...
      {inCall && (
        <div style={{ position:"fixed", inset:0, background:"#0b141a", zIndex:9997, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:20 }}>
          ...
          <div style={{ color:"#8696a0", fontSize:14 }}>Call jari hai...</div>
      ...
      {showInvite && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:9996, display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#202c33", borderRadius:20, padding:28, maxWidth:340, width:"90%", textAlign:"center" }}>
            ...
            <h3 style={{ color:"#e9edef", margin:"0 0 8px" }}>Dost ko Invite Karein</h3>
            <p style={{ color:"#8696a0", fontSize:13, marginBottom:16 }}>Ye link copy karke dost ko bhejein!</p>
            ...
            <div onClick={() => setShowInvite(false)} style={{ padding:"10px", color:"#8696a0", cursor:"pointer" }}>Band Karein</div>
          </div>
        </div>
      )}
      ...
      <div style={{ width:340, minWidth:340, display:"flex", flexDirection:"column", borderRight:"1px solid #1f2c33" }}>
        ...
        {showNewChat && (
          <div style={{ padding:"10px 12px", background:"#182229", borderBottom:"1px solid #1f2c33" }}>
            <div style={{ fontSize:13, color:"#25D366", fontWeight:600, marginBottom:8 }}>Naya Chat</div>
            <input value={newChatEmail} onChange={e => setNewChatEmail(e.target.value)} placeholder="Dost ki email..."
            ...
            <div style={{ display:"flex", gap:8 }}>
              <div onClick={startChat} style={{ flex:1, padding:"9px", background:"#25D366", borderRadius:10, textAlign:"center", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer" }}>Chat Shuru</div>
              <div onClick={() => { setShowNewChat(false); setNewChatError(""); }} style={{ padding:"9px 14px", background:"#2a3942", borderRadius:10, color:"#8696a0", cursor:"pointer" }}>X</div>
            </div>
          </div>
        )}
        ...
        <div style={{ flex:1, overflowY:"auto" }}>
          {Object.keys(contacts).length === 0 ? (
            <div style={{ padding:24, textAlign:"center", color:"#8696a0" }}>
              <div style={{ fontSize:40, marginBottom:12 }}>👥</div>
              <div style={{ fontSize:14 }}>Koi contact nahi</div>
              <div style={{ fontSize:12, marginTop:6 }}>✏️ ya 🔗 se shuru karein</div>
            </div>
      ...
      {activeChat ? (
        <div style={{ flex:1, display:"flex", flexDirection:"column", background:"#0b141a" }}>
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"14px 6%" }}>
            {messages.length === 0 && (
              <div style={{ textAlign:"center", marginTop:60, color:"#8696a0" }}>
                ...
                <div>Salam! {activeChat.name} se baat shuru karein</div>
              </div>
            )}
            {messages.map((msg, i) => {
          ...
          <div style={{ display:"flex", alignItems:"center", gap:9, padding:"8px 12px", background:"#202c33" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", background:"#2a3942", borderRadius:26, flex:1, padding:"9px 14px", gap:10 }}>
              <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key==="Enter" && !e.shiftKey && sendMessage()} placeholder="Message likhein..."
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#0b141a", gap:18, padding:24 }}>
          ...
          <p style={{ color:"#8696a0", fontSize:14, textAlign:"center", maxWidth:320, lineHeight:1.7, margin:0 }}>
            Doston se real-time chat, audio aur video call karein!
          </p>
